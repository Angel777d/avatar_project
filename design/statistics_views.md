...
`build_snapshot` returns one dict for the whole page; a range change asks the app to move and the page renders whatever is pushed back. Aggregation walks the log twice — once for spans, once for events — and nothing else, so a year of records is still a single pass over a few thousand rows.
